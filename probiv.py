# -*- coding: utf-8 -*-
# Hikka модуль: TeledoX Pro v5 (финальная оптимизированная версия)
...
@loader.tds
class TeledoXProV5Mod(loader.Module):
    ...
    async def _navigate_menu(self, menu_flow):
        ...
        for btn_text in menu_flow:
            ...
            if not success:
                logger.warning(f"Button not found: {btn_text}")
                # Попытка повторного клика через секунду
